...
def split_data(cleantweet):
  trainX, tempX = train_test_split(cleantweet, test_size=0.3, random_state=42)
  valX, testX = train_test_split(tempX, test_size=0.5, random_state=42)
  return trainX, valX, testX

#extract tweet and y
def extract_tweet_and_y(raw_data_df):
  tweet, target = raw_data_df['clean_tweet'], raw_data_df['class'] #change class to hate
  return tweet, target
...
def albert_model(param={}):
  ...
  model.add(inputs)
  model.add(e)
  model.add(Bidirectional(LSTM(100, return_sequences=True, dropout=param['dropout']), merge_mode='concat')) #change LSTM 100 (352) to GRU, param['dropout'], 0.65
  model.add(Bidirectional(LSTM(100, return_sequences=True, dropout=param['dropout']),merge_mode='concat')) #change LSTM 100 (320) to GRU, param['dropout'], 0.80
  model.add(Flatten())
  ...
  model.add(Dropout(param['dropout'])) #param['dropout'], 0.35, change
...
def compile_model(model):
  model.compile(loss='categorical_crossentropy', optimizer='adam', #optimizer = keras.optimizers.Nadam(learning_rate=0.0001),optimizer='adam'  change
                metrics=['accuracy', 
