import pickle #change
from Preprocessing_helper import *
from Data_Model_helper import *
from Data_Augmentation_n_Sentiment_helper import *
from imblearn.over_sampling import RandomOverSampler
from imblearn.under_sampling import RandomUnderSampler

class Albert(object):
    def __init__(self, data, batch_size, num_epochs): #change
        self.data = data
        self.num_epochs = num_epochs
        self.batch_size = batch_size

    def fit_albert(self):
      clean_data = preprocessing_tweet(self.data)
      
      train_df1, val_df, test_df = split_data(clean_data)
      train_df=apply_oversampling(train_df1)
      X_train, y_train = extract_tweet_and_y(train_df)
      X_val, y_val = extract_tweet_and_y(val_df)
      X_test, y_test = extract_tweet_and_y(test_df)

      y_raw, class_weight_raw = prepare_target(clean_data['class'])
      y_train, class_weight_train = prepare_target(y_train)
      y_val, class_weight_val = prepare_target(y_val)
      y_test, class_weight_test = prepare_target(y_test)
    
      #Albert Model Tokenizer
      X_train_albert, X_val_albert, X_test_albert, vocab_size, word_index = keras_tokenizer(X_train,X_val,X_test, maxnumwords=100)

      #Use GloVe or None Embedding
      embedding_matrix = None #change
      
    #   embedding_matrix = GloveTwitterEmbedding(vocab_size, word_index) #change
    
      #Albert Model
      Albertmodel = albert_model(param={'Max_length': 100,
                                        'Vocab Size': vocab_size,
                                        'Embedding Matrix': embedding_matrix,
                                        'dropout':0.20, #change was 0.20
                                        'first_layer' : 128, #change was 288, 128
                                        'second_layer' : 64, #change was 416, 64
                                        })
      Albertmodel = compile_model(Albertmodel)

      self.Albertmodel, history_Albert = train_model(Albertmodel, X_train_albert, y_train, X_val_albert, y_val, batch_size=self.batch_size, num_epochs=self.num_epochs, class_weight=class_weight_train)
          
      probs = self.Albertmodel.predict(X_test_albert)
      print("#"*100)
      print("----------------------------      Evaluation on Testing Data     -----------------------------------")
      print("#"*100)
      print(" ")
      plot_confusion_matrix(probs, y_test)
      print_classification_report(probs, y_test)
    
      # Saving For Plottting, change
      with open('/content/drive/My Drive/Advanced_ML_Project/Offensive-Language-Detection-DL/Offensive-Language-Detection-DL/history.pkl', 'wb') as file_pi:
          pickle.dump(history_Albert.history, file_pi)
      y_pred = np.argmax(probs, axis=1) #change
      class_names = ['Hate', 'Offensive', 'Neither'] #change
      # Saving For Plottting, change
      data_to_save = {
          'y_true': y_test,  # One-hot encoded true labels
          'y_scores': probs,  # Predicted probabilities for each class
          'y_pred': y_pred,  # Predicted class indices
          'class_names': class_names  # List of class names for confusion matrix
      }
      with open('/content/drive/My Drive/Advanced_ML_Project/Offensive-Language-Detection-DL/Offensive-Language-Detection-DL/roc_confusion_data.pkl', 'wb') as f:
          pickle.dump(data_to_save, f)

    def predict(self, New_tweet):
      
      if isinstance(New_tweet, list):
        kt = Tokenizer()
        kt.fit_on_texts(New_tweet)
        tweet_vectors = kt.texts_to_sequences(New_tweet) #Converting text to a vector of word indexes
        tweet_padded = pad_sequences(tweet_vectors, maxlen=100, padding='post')
        self.prediction_prods = self.Albertmodel.predict(tweet_padded)
        self.prediction = self.prediction_prods.argmax(1)
        if self.prediction[0]==1 & len(New_tweet) ==1:
          print("This is classified as Offensive")
        elif self.prediction[0] ==0 & len(New_tweet) ==1:
          print("This is classified as Hate")
        elif self.prediction[0] ==2 & len(New_tweet) ==1:
          print("This is classified as Neither Offensive nor Hate")

      elif isinstance(New_tweet, pd.core.series.Series):
        kt = Tokenizer()
        kt.fit_on_texts(New_tweet)
        tweet_vectors = kt.texts_to_sequences(New_tweet) #Converting text to a vector of word indexes
        tweet_padded = pad_sequences(tweet_vectors, maxlen=100, padding='post')
        self.prediction_prods = self.Albertmodel.predict(tweet_padded)
        self.prediction = self.prediction_prods.argmax(1)

      elif isinstance(New_tweet, str):
        kt = Tokenizer()
        kt.fit_on_texts([New_tweet])
        tweet_vectors = kt.texts_to_sequences([New_tweet]) #Converting text to a vector of word indexes
        tweet_padded = pad_sequences(tweet_vectors, maxlen=100, padding='post')
        self.prediction_prods = self.Albertmodel.predict(tweet_padded)
        self.prediction = self.prediction_prods.argmax(1)
        if self.prediction[0]==1:
          print("This is classified as Offensive")
        elif self.prediction[0] ==0:
          print("This is classified as Hate")
        elif self.prediction[0] ==2:
          print("This is classified as Neither Offensive nor Hate")
          

      elif isinstance(New_tweet, pd.core.frame.DataFrame):
        kt = Tokenizer()
        kt.fit_on_texts(New_tweet.values.tolist())
        tweet_vectors = kt.texts_to_sequences(New_tweet.values.tolist()) #Converting text to a vector of word indexes
        tweet_padded = pad_sequences(tweet_vectors, maxlen=100, padding='post')
        self.prediction_prods = self.Albertmodel.predict(tweet_padded)
        self.prediction = self.prediction_prods.argmax(1)

      else:
        print("Error!\nInput format is not support. Please try other format")
        
    def check_sentiment(self, New_tweet):
      
      if isinstance(New_tweet, list):
        Albert_Sentiment(New_tweet[0])

      elif isinstance(New_tweet, str):
        Albert_Sentiment(New_tweet)

      else:
        print("Error!\nInput format is not support. Please try other format")

    def corpus_augmentation(self, dataframe, class_label, number_of_augmentation):
      self.corpus_augmentation = docs_augment(dataframe, class_label, number_of_augmentation)

    def doc_augmentation(self, New_tweet):
      doc_aug = data_augment_bert_sw(aug_insert_bert, aug_substitute_bert, aug_swap, New_tweet)
      print("Original Text:")
      print(New_tweet)
      print("Augmented Text:")
      print(doc_aug)
      self.doc_augmentation =doc_aug 
        
        
class Albert_pretrain(object):
    # def __init__(self, data):
    #     self.data = data

    def load_albert(self):
      # Recreate the exact same model, including its weights and the optimizer
      self.Albertmodel = tf.keras.models.load_model('../model/albert_model.h5')

      print("#"*100)
      print("------------------------      Albert Pretrain Model Loaded Successfully     -----------------------")
      print("#"*100)
      print("Below is model summary")
      # Show the model architecture
      self.Albertmodel.summary()

    def predict(self, New_tweet):
      
      if isinstance(New_tweet, list):
        kt = Tokenizer()
        kt.fit_on_texts(New_tweet)
        tweet_vectors = kt.texts_to_sequences(New_tweet) #Converting text to a vector of word indexes
        tweet_padded = pad_sequences(tweet_vectors, maxlen=100, padding='post')
        self.prediction_prods = self.Albertmodel.predict(tweet_padded)
        self.prediction = self.prediction_prods.argmax(1)
        if self.prediction[0]==1 & len(New_tweet) ==1:
          print("This is classified as Offensive")
        elif self.prediction[0] ==0 & len(New_tweet) ==1:
          print("This is classified as Hate")
        elif self.prediction[0] ==2 & len(New_tweet) ==1:
          print("This is classified as Neither Offensive nor Hate")

      elif isinstance(New_tweet, pd.core.series.Series):
        kt = Tokenizer()
        kt.fit_on_texts(New_tweet)
        tweet_vectors = kt.texts_to_sequences(New_tweet) #Converting text to a vector of word indexes
        tweet_padded = pad_sequences(tweet_vectors, maxlen=100, padding='post')
        self.prediction_prods = self.Albertmodel.predict(tweet_padded)
        self.prediction = self.prediction_prods.argmax(1)

      elif isinstance(New_tweet, str):
        kt = Tokenizer()
        kt.fit_on_texts([New_tweet])
        tweet_vectors = kt.texts_to_sequences([New_tweet]) #Converting text to a vector of word indexes
        tweet_padded = pad_sequences(tweet_vectors, maxlen=100, padding='post')
        self.prediction_prods = self.Albertmodel.predict(tweet_padded)
        self.prediction = self.prediction_prods.argmax(1)
        if self.prediction[0]==1:
          print("This is classified as Offensive")
        elif self.prediction[0] ==0:
          print("This is classified as Hate")
        elif self.prediction[0] ==2:
          print("This is classified as Neither Offensive nor Hate")

      elif isinstance(New_tweet, pd.core.frame.DataFrame):
        kt = Tokenizer()
        kt.fit_on_texts(New_tweet.values.tolist())
        tweet_vectors = kt.texts_to_sequences(New_tweet.values.tolist()) #Converting text to a vector of word indexes
        tweet_padded = pad_sequences(tweet_vectors, maxlen=100, padding='post')
        self.prediction_prods = self.Albertmodel.predict(tweet_padded)
        self.prediction = self.prediction_prods.argmax(1)

      else:
        print("Error!\nInput format is not support. Please try other format")
        
    def check_sentiment(self, New_tweet):
      
      if isinstance(New_tweet, list):
        Albert_Sentiment(New_tweet[0])

      elif isinstance(New_tweet, str):
        Albert_Sentiment(New_tweet)

      else:
        print("Error!\nInput format is not support. Please try other format")

    def corpus_augmentation(self, dataframe, class_label, number_of_augmentation):
      self.corpus_augmentation = docs_augment(dataframe, class_label, number_of_augmentation)


    def doc_augmentation(self, New_tweet):
      doc_aug = data_augment_bert_sw(aug_insert_bert, aug_substitute_bert, aug_swap, New_tweet)
      print("Original Text:")
      print(New_tweet)
      print("Augmented Text:")
      print(doc_aug)
      self.doc_augmentation =doc_aug 
      
def apply_oversampling(df):
    X = df['clean_tweet']  # Features (text)
    y = df['class']  # Target labels (0 = hate_speech, 1 = offensive, 2 = none)
    
    # Initialize RandomOverSampler with specific sampling strategy
    oversampler = RandomOverSampler(sampling_strategy={
        0: 4000,  # Oversample hate_speech (label 0) to 10,000 samples
        2: 10000   # Oversample none (label 2) to 15,000 samples
    })
    
    # Apply resampling
    X_resampled, y_resampled = oversampler.fit_resample(X.values.reshape(-1, 1), y)
    
    # Create a DataFrame with the resampled data
    df_resampled = pd.DataFrame({'clean_tweet': X_resampled.flatten(), 'class': y_resampled})
    
    print(f"Class distribution after oversampling:\n{df_resampled['class'].value_counts()}")
    
    return df_resampled
    

def apply_equal_over_resampling(df):
    X = df['clean_tweet']  # Features (text)
    y = df['class']  # Target labels (0 = hate_speech, 1 = offensive, 2 = none)
    
    # Find the minimum class size
    min_class_size = y.value_counts().min()
    
    # Initialize RandomOverSampler to balance the dataset by oversampling the minority class
    resampler = RandomOverSampler(sampling_strategy='auto')  # Use min_class_size to equalize all classes
    
    # Apply resampling
    X_resampled, y_resampled = resampler.fit_resample(X.values.reshape(-1, 1), y)
    
    # Create a DataFrame with the resampled data
    df_resampled = pd.DataFrame({'clean_tweet': X_resampled.flatten(), 'class': y_resampled})
    
    # Print class distribution after equal resampling
    print(f"Class distribution after equal resampling:\n{df_resampled['class'].value_counts()}")
    
    return df_resampled
    
def apply_equal_under_resampling(df):
    X = df['clean_tweet']  # Features (text)
    y = df['class']  # Target labels (0 = hate_speech, 1 = offensive, 2 = none)
    
    # Find the minimum class size
    min_class_size = y.value_counts().min()
    
    # Initialize RandomOverSampler to balance the dataset by oversampling the minority class
    resampler = RandomUnderSampler(sampling_strategy='auto')  # Use min_class_size to equalize all classes
    
    # Apply resampling
    X_resampled, y_resampled = resampler.fit_resample(X.values.reshape(-1, 1), y)
    
    # Create a DataFrame with the resampled data
    df_resampled = pd.DataFrame({'clean_tweet': X_resampled.flatten(), 'class': y_resampled})
    
    # Print class distribution after equal resampling
    print(f"Class distribution after equal resampling:\n{df_resampled['class'].value_counts()}")
    
    return df_resampled

def apply_oversample_and_undersample(df):
    X = df['tweet']  # Features (text)
    y = df['class']  # Target labels (0 = hate_speech, 1 = offensive, 2 = none)
    
    # Step 1: Oversample the minority classes (0 and 2) to 5000 samples
    oversampler = RandomOverSampler(sampling_strategy={0: 10000, 2: 10000})  # Oversample class 0 and class 2 to 5000 samples
    X_oversampled, y_oversampled = oversampler.fit_resample(X.values.reshape(-1, 1), y)
    
    # Step 2: Undersample all classes to 5000 samples
    undersampler = RandomUnderSampler(sampling_strategy='auto')  # Undersample all classes to 5000 samples
    X_resampled, y_resampled = undersampler.fit_resample(X_oversampled, y_oversampled)
    
    # Create a DataFrame with the resampled data
    df_resampled = pd.DataFrame({'clean_tweet': X_resampled.flatten(), 'class': y_resampled})
    
    # Print class distribution after resampling
    print(f"Class distribution after resampling:\n{df_resampled['class'].value_counts()}")
    
    return df_resampled

if __name__ == "__main__":
    # Pretrain = Albert_pretrain()
    # Pretrain.load_albert()
    # Pretrain.predict("I like it very very much")

    epoch = 50 #change
    batch_size = 128
    df=load_data()
    df_balanced=apply_oversample_and_undersample(df)
    #df_balanced = apply_oversampling(df)
    #print(f"Class distribution in df_balanced:\n{df_balanced['class'].value_counts()}")
    Albert = Albert(df_balanced, batch_size = batch_size, num_epochs=epoch)
    Albert.fit_albert()
    Albert.predict("How are you today")
    Albert.predict("I really hit this one fuck die die die")
    Albert.predict("I really hate him so much and do not like him at all")

